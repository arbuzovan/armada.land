uAdmin('.eip', function (extend) {
	...
	uEditInPlace.prototype.initEditBoxes = function () {
		...
		jQuery('.u-eip-edit-box-hover').live('mouseout', function () {
	...
	uEditInPlace.prototype.addDeleteButton = function (element, info) {
		...
		if (this.deleteButtonsTimeout) clearTimeout(this.deleteButtonsTimeout);
		...
		jQuery(deleteButton)
			.bind('mouseover', function () {
				if (self.deleteButtonsTimeout) clearTimeout(self.deleteButtonsTimeout);
			})
	...
	uEditInPlace.prototype.addSaveButton = function () {
		var self = this;
		if (!this.editorControlsHolder) return null;
		return jQuery('\n\
	...
	uEditInPlace.prototype.metaInit = function () {
		...
		this.meta.old = {
			alt_name     : uAdmin.data.page['alt-name'] ? uAdmin.data.page['alt-name'] : '',
			title        : uAdmin.data.title ? uAdmin.data.title : '',
			keywords     : uAdmin.data.meta.keywords ? uAdmin.data.meta.keywords : '',
			descriptions : uAdmin.data.meta.description ? uAdmin.data.meta.description : ''
		};
		this.meta['new'] = {};
	...
	uEditInPlace.prototype.inlineAddPage = function (node) {
		...
		var cleanTags = function (node) {
			...
			if (jQuery(node).attr('umi:field-name')) {
				if (jQuery.inArray(jQuery(node).attr('umi:field-name'), arTypeFields) == -1) return false;
	...
	uEditInPlace.prototype.onSuccessAddNewPage = function(pageData){
		...
		var replaceAttributes = function (jqElement, data) {
			...
			jqElement.children().each(function(){
				replaceAttributes(jQuery(this), data);
			});
		};

		jqParents.each(function(){
			var jqParent = jQuery(this);
			var jqNewItem = jqParent.find("[umi\\:region=row]").first().clone();
			replaceAttributes(jqNewItem, pageData);
			jqNewItem.appendTo(jqParent);
		});
...
//			if(jQuery(n).attr('umi:module') != module) return;		// module не определен
						...
						jQuery("[umi\\:"+node.type+"-id="+parentId+"][umi\\:region=list]")
							...
							.each(function(){
								...
								var parentNode = jQuery(this),
									...
									linkItem = addedNode.find('*').andSelf().filter("[umi\\:url-attribute]").first(); // element that contains 'umi:url-attribute'

								if (linkItem.length) {
									var urlAttribute = linkItem.attr('umi:url-attribute'),
										newLinkItem = newItem.find('*').andSelf();
						...
						jQuery.post('/admin/content/editValue/save.json', params, function (data) {
							...
							if (oldLink && newLink) {
								...
								rootNodes.each(function(){
									jQuery(this).find('[umi\\:url-attribute]').each(function(){
										jQuery(this).attr(jQuery(this).attr('umi:url-attribute'), newLink).bind('click mousedown mouseup', function(){ return true });
										iProcessedNodes++;
									});
								});
								if (!iProcessedNodes) {
									rootNodes.find('a').each(function (i, n) {
										jQuery(this).attr('href', newLink).bind('click mousedown mouseup', function(){ return true });
									});
								};
							};
							jQuery("[umi\\:"+node.type+"-id='"+node.id+"']")
								...
								.each(function(){
									...
									if (elem.is('img')) {
										elem.attr('src', data.property['value']['src']);
									} else {
