uAdmin('.ieditor', function (extend) {
	...
	var settings = {
		...
		module: {
			...
			edit_module: {
				css_class: 'eip-ieditor-module-edit'
			},
			crop_module: {
				css_class: 'eip-ieditor-module-crop'
			},
			resize_module: {
				css_class: 'eip-ieditor-module-resize'
			},
			rotate_module: {
				css_class: 'eip-ieditor-module-rotate'
			},
			popup_module: {
		...
		browser_modules: {
			msie: [FilemanagerModule, SliderModule, EditModule, RotateModule, PopupModule, ApplyModule, CancelModule, DeleteModule],
			mozilla: [FilemanagerModule, SliderModule, UploadModule, EditModule, RotateModule, ResizeModule, CropModule, PopupModule, ApplyModule, CancelModule, DeleteModule],
			opera: [FilemanagerModule, SliderModule, UploadModule, EditModule, RotateModule, ResizeModule, CropModule, PopupModule, ApplyModule, CancelModule, DeleteModule],
			webkit: [FilemanagerModule, SliderModule, UploadModule, EditModule, RotateModule, ResizeModule, CropModule, PopupModule, ApplyModule, CancelModule, DeleteModule]
		}
	...
	var uImageEditor = function () {
		...
		uAdmin.eip.bind('AddPhotoToAlbum', function(type, options){
			if (type == 'after' && options && options.newNode && options.newNode.length) {
				...
				options.newNode.find("img[" + sAttr + "]").andSelf().removeAttr(sAttr);
	...
	uImageEditor.prototype.saveStateToCockie = function () {
		if (!jQuery.cookie) return;
	...
	ImageEditorBase.prototype.drawModules = function (arModules) {
		...
		for (var i in this.arModulesSet) {
			...
			if (bIsStub && this.arModulesSet[i] == EditModule) continue;
			// disable some modules on IE
			if (jQuery.browser.msie && (this.arModulesSet[i] == ResizeModule || this.arModulesSet[i] == CropModule || this.arModulesSet[i] == UploadModule)) continue;
			// end disable
	...
	ImageEditorBase.prototype.switchOn = function () {
		var self = this;
		this.jqImgWrapper
	...
	ImageEditorBase.prototype.checkModule = function (Module) {
		var skipEditModule = Module == EditModule && this.checkImageIsEmpty(),
		...
		return typeof Module === 'function' && !!Module.isImageEditorModule && !skipEditModule && enabledInBrowser;
	};
	...
	ImageEditorBase.prototype.isPopupThumb = function () {
		if (typeof PopupModule == 'undefined') return false;
	...
	ImageEditorBase.prototype.isStub = function () {
		var bAttrCondition = this.jqImgNode.attr("umi:is-stub") == "true",
			bImgCondition = !(jQuery.browser.msie && jQuery.browser.version < 10) && this.jqImgNode[0] && this.jqImgNode[0].complete && !this.jqImgNode[0].naturalWidth && !this.jqImgNode[0].naturalHeight;
	...
	ImageEditorBase.prototype.setEnabledModule = function (oModule) {
		...
		if (!this.deactivateEnabledModule(oModule != EditModule)) {
			return false;
		}
	...
	ImageEditorBase.prototype.setEditMode = function (arModules) {
		if (EditModule) {
			var oEditModule = new EditModule(this);
			oEditModule.activate(arModules);
		}
	};

	ImageEditorBase.prototype.setEditState = function (bState) {
		bState ? ImageEditorsCollection.getInstance().turnOnEditMode() : ImageEditorsCollection.getInstance().turnOffEditMode();
		this.bEditStateEnabled = !!bState;
	};

	ImageEditorBase.prototype.getEditState = function () {
		return this.bEditStateEnabled;
	};
	...
	ImageEditorVoid.prototype.setEditMode = function () {};
	ImageEditorVoid.prototype.setEditState = function () {};
	ImageEditorVoid.prototype.getEditState = function () {};
		...
		this.jqIframe = jQuery("<iframe/>")
			...
			.on("load", function(){
				...
				if (!sImgPath) {
					...
					var sOldValue = self.oEditor.jqImgNode.attr('src');
					self.setNewImage(sImgPath);
					self.oEditor.setEditMode();
					self.addToEipQueue(sImgPath, sOldValue);
				}
				self.cleanup();
			});
	...
	SliderModule.prototype.activate = function () {
		...
		var $slideNode = $(this.oEditor.jqImgNode[0]);

		var queryParams = {
			'slider_id': $slideNode.attr('umi:slider-id'),
			'token': uAdmin['csrf'],
		...
		$.openPopupLayer({
			...
			'url':'/styles/common/other/slidereditor/slideEditor.html?' + $.param(queryParams),
			'width':710,
			'height': 700,
			'success': function(){
	...
	FilemanagerModule.prototype.activate = function () {
		...
		this.getImageFromFilemanager(function (arFiles) {
			...
			self.oEditor.setEditMode();
			self.addToEipQueue(arFiles[0], sOldValue);
		});
		window.setTimeout(function(){
			self.oEditor.switchOn();
			if (jQuery.browser.webkit) {
	...
	function EditModule (oEditor) {

		EditModule.superclass.constructor.call(this);

		this.title = getLabel("js-ieditor-module-edit-title");

		if (oEditor.iImageType == oEditor.IMAGE_TYPE_SLIDER){
			this.cssClass = 'eip-ieditor-empty';
		} else {
			this.cssClass = settings.module.edit_module.css_class;
		}

		this.arEditorModules = [
			CropModule,
			RotateModule
		];
	...
	uAdmin.inherit(EditModule, ModuleBase);

	EditModule.isImageEditorModule = true;

	EditModule.prototype.activate = function (arModules) {
		EditModule.superclass.activate.call(this);
		...
		if (this.oEditor.iImageType === this.oEditor.IMAGE_TYPE_WYSIWYG) {
			if (!tinymce.activeEditor) {
				...
				this.oEditor.reinit();
				var sEditorId = this.oEditor.jqImgNode.attr(ImageEditorsCollection.getInstance().EDITOR_ID_ATTRIBUTE),
					sImageSrc = this.oEditor.jqImgNode.attr('src'),
					waitForEditor = window.setInterval(function(){
						if (!tinymce.activeEditor || !tinymce.activeEditor.initialized) return;
						var imgNode = tinymce.activeEditor.dom.select('img[src="'+sImageSrc+'"]')[0],
							jqImgNode = jQuery(imgNode),
							oImgEditor = ImageEditorsCollection.getInstance().initEditor(imgNode);
						if (oImgEditor instanceof ImageEditorBase == false) return;
						oImgEditor.destroy(true);
						oImgEditor.setEditState(true);
						oImgEditor.init(jqImgNode, jqImgNode.attr(ImageEditorsCollection.getInstance().EDITOR_ID_ATTRIBUTE));
						window.clearInterval(waitForEditor);
					}, 500);
				this.oEditor.jqImgNode.click();
				document.body.scrollTop = iScrollTop;
				return;
			}
			this.arEditorModules.push(ResizeModule);
			this.arEditorModules.push(PopupModule);
		}
		this.oEditor.reinit();
		this.oEditor.hide();
		this.oEditor.redrawMenu(arModules || this.arEditorModules);
		this.oEditor.removeDeleteButton();
		this.oEditor.addCloseButton(function(oEvent){
			self.oEditor.switchOn();
			self.oEditor.setEditState(false);
			self.oEditor.removeCloseButton();
			self.oEditor.reinit();
			self.oEditor.redrawMenu();
			self.oEditor.drawDeleteButton(true);
			oEvent.preventDefault();
			oEvent.stopPropagation();
			oEvent.stopImmediatePropagation();
			return false;
		...
		window.setTimeout(function(){
			self.oEditor.switchOff();
			self.oEditor.setEditState(true);
			self.oEditor.show();
		}, 100);
	};


	function CropModule (oEditor) {

		CropModule.superclass.constructor.call(this);

		this.title = getLabel("js-ieditor-module-crop-title");
		this.cssClass = settings.module.crop_module.css_class;

		this.oOriginalImageData = {
			path: '',
			scale: 1,
			top: 0,
			left: 0,
		...
		this.sImgAreaSelectWrapperCssClass = settings.img_area_select.css_class;
		this.jqImgAreaSelectWrapper = null;
		this.imgAreaSelect = null;
		this.imgAreaSelectOptions = jQuery.extend({}, settings.img_area_select.plugin_options, {
			onSelectEnd: function () {
				// Chrome schedules unwanted click event right after mouseup (which fires this callback)
				// Set skip flag for click handler and schedule new callback right after handler to restore flag value.
				var ieditor = uAdmin.ieditor;
				ieditor.skipDeactivateClick = true;
				window.setTimeout(function() {
					ieditor.skipDeactivateClick = false;
				}, 0);
			}
		});
		this.imgAreaSelectOptions.parent = this.jqImgAreaSelectWrapper;

		this.jqOriginalImage = null;
		this.originalRepositionHandler = null;

		this.init(oEditor);

	};
	uAdmin.inherit(CropModule, ModuleBase);

	CropModule.isImageEditorModule = true;

	CropModule.prototype.init = function (oEditor) {

		CropModule.superclass.init.call(this);
		this.oEditor = oEditor;

	};

	CropModule.prototype.activate = function () {
		CropModule.superclass.activate.call(this);

		var self = this;

		if (!jQuery.imgAreaSelect) return;

		this.originalRepositionHandler = this.oEditor.reposition;
		this.oEditor.reposition = function () {
			self.originalRepositionHandler.call(this);
			this.jqMenuWrapper.css({
				bottom: -10 - this.jqMenuWrapper.height() + 'px'
			});
		};

		this.oEditor.hide();
		this.oEditor.redrawMenu([ApplyModule, CancelModule]);
		this.oEditor.removeCloseButton();
		this.oEditor.switchOff();
		this.oEditor.reposition();
		this.oEditor.show();

		this.oEditor.showPreloader();
		this.initOriginalImageData();

		this.drawOriginalImage();

	};

	CropModule.prototype.save = function () {
		if (!this.imgAreaSelect) return;
		CropModule.superclass.deactivate.call(this);

		var oData = this.imgAreaSelect.getSelection();
		oData['scale'] = this.oOriginalImageData.scale;
		oData['displayWidth'] = this.oOriginalImageData['displayWidth'];
		oData['displayHeight'] = this.oOriginalImageData['displayHeight'];
		...
		this.process('crop', oData);

		this.cleanup();
	};

	CropModule.prototype.cancel = function () {
		CropModule.superclass.deactivate.call(this);
		this.cleanup();
	};

	CropModule.prototype.cleanup = function () {
		this.oEditor.reposition = this.originalRepositionHandler;
		this.removeSelection();
		this.jqImgAreaSelectWrapper.remove();
		...
		var self = this;
		window.setTimeout(function(){
			if (self.oEditor.getEnabledModule() instanceof EditModule === false) {
				var oEditModule = new EditModule(self.oEditor);
				oEditModule.activate();
			}
		}, 0);
	};

	CropModule.prototype.initOriginalImageData = function () {
		if (this.oOriginalImageData && this.oOriginalImageData.path && this.oOriginalImageData.path != '') return this.oOriginalImageData;
		var oImgElementInfo = this.oEditor.getEipInfo();
		oImgElementInfo['image_url'] = this.oEditor.jqImgNode.attr('src');
		delete oImgElementInfo['node'];
		var oRequesrResult = jQuery.ajax({
			async: false,
			url: settings.image_data_url,
			type: settings.image_data_request_method,
			data: oImgElementInfo
		});
		var data = jQuery.parseJSON(oRequesrResult.responseText);
		if (data.path && data.path != '') {
			delete data['method'];
			delete data['module'];
			this.oOriginalImageData = data;
			return this.oOriginalImageData;
		}
		return false;
	};

	CropModule.prototype.drawOriginalImage = function () {
		var self = this;

		this.jqImgAreaSelectWrapper = jQuery("<div/>")
			.addClass(this.sImgAreaSelectWrapperCssClass)
			.css({
				display: 'none',
				position: 'absolute',
				top: this.oEditor.jqImgNode.css('paddingTop') || 0,
				left: this.oEditor.jqImgNode.css('paddingLeft') || 0,
				zIndex: 100000
			})
			.offset(this.oEditor.jqImgNode.offset())
			.appendTo("body");
		this.jqOriginalImage = jQuery("<img/>")
			.attr({
				src: this.oOriginalImageData.path + '?' + new Date().getMilliseconds()
			})
			.css({
				width: this.oOriginalImageData.naturalWidth || '',
				height: this.oOriginalImageData.naturalHeight || ''
			})
			.appendTo(this.jqImgAreaSelectWrapper)
			.one("load", function(){
				self.jqImgAreaSelectWrapper.show();
				self.repositionOriginalImage();
				self.setSelection();
				self.imgAreaSelect.show();
				self.repositionMenu();
				self.oEditor.hidePreloader();
			});
		this.imgAreaSelectOptions.parent = this.jqImgAreaSelectWrapper;
	...
	CropModule.prototype.repositionOriginalImage = function () {
		var iOriginalWidth = 0,
			iTargetWidth = 0,
			iOriginalHeight = 0,
			iTargetHeight = 0,
			iScale = 1;

		iOriginalWidth = this.oOriginalImageData.naturalWidth;
		iOriginalHeight = this.oOriginalImageData.naturalHeight;

		if (this.oOriginalImageData.width) {
			iScale = this.oEditor.jqImgNode.width() / this.oOriginalImageData.width;
		} else {
			iScale = this.oEditor.jqImgNode.width() / iOriginalWidth;
		}
		this.oOriginalImageData['scale'] = iScale;

		iTargetWidth = iOriginalWidth * iScale;
		iTargetHeight = iOriginalHeight * iScale;
		this.jqOriginalImage.css({
			width: iTargetWidth,
			maxWidth: iTargetWidth,
			height: iTargetHeight,
			maxHeight: iTargetHeight
		});
		this.jqImgAreaSelectWrapper.width(iTargetWidth).height(iTargetHeight).offset({
			left: parseInt(this.oEditor.jqImgNode.css('paddingLeft')) + this.oEditor.jqImgNode.offset().left - (this.oOriginalImageData.left * iScale || 0),
			top: parseInt(this.oEditor.jqImgNode.css('paddingTop')) + this.oEditor.jqImgNode.offset().top - (this.oOriginalImageData.top * iScale || 0)
		});
		this.oOriginalImageData['displayWidth'] = this.jqOriginalImage.width();
		this.oOriginalImageData['displayHeight'] = this.jqOriginalImage.height();
	};

	CropModule.prototype.repositionMenu = function () {
		if (this.jqOriginalImage.outerHeight(true) <= this.oEditor.jqImgNode.outerHeight(true)) return;
		this.oEditor.applyImageSizes(this.jqOriginalImage);
		this.oEditor.reposition();
	};

	CropModule.prototype.setSelection = function () {
		this.imgAreaSelect = jQuery(this.jqOriginalImage).imgAreaSelect(this.imgAreaSelectOptions);
		var x1 = this.oOriginalImageData['left'] * this.oOriginalImageData['scale'] || 0,
			y1 = this.oOriginalImageData['top'] * this.oOriginalImageData['scale'] || 0,
			x2 = x1 + (this.oOriginalImageData['width'] * this.oOriginalImageData['scale'] || this.oEditor.jqImgNode.width() || 0),
			y2 = y1 + (this.oOriginalImageData['height'] * this.oOriginalImageData['scale'] || this.oEditor.jqImgNode.height() || 0);
		this.imgAreaSelect.setSelection(x1, y1, x2, y2);
		this.imgAreaSelect.setMode('crop');
	};

	CropModule.prototype.removeSelection = function () {
		this.jqOriginalImage.imgAreaSelect({remove: true});
		this.imgAreaSelect = null;
	};


	function ResizeModule (oEditor) {

		ResizeModule.superclass.constructor.call(this);

		this.title = getLabel("js-ieditor-module-resize-title");
		this.cssClass = settings.module.resize_module.css_class;

		this.imgAreaSelect = null;
		this.imgAreaSelectOptions = jQuery.extend({}, settings.img_area_select.plugin_options);
		this.imgAreaSelectOptions.movable = false;
		this.imgAreaSelectOptions.parent = "body";
		this.oInitialSize = {
			width: 0,
			height: 0
		};
	...
	uAdmin.inherit(ResizeModule, ModuleBase);

	ResizeModule.isImageEditorModule = true;

	ResizeModule.prototype.init = function (oEditor) {
		ResizeModule.superclass.init.call(this);
		this.oEditor = oEditor;
		this.imgAreaSelectOptions.aspectRatio = this.oEditor.jqImgNode.width() + ":" + this.oEditor.jqImgNode.height();
	};

	ResizeModule.prototype.activate = function () {
		ResizeModule.superclass.activate.call(this);

		var self = this;

		this.oInitialSize.width = this.oEditor.jqImgNode.width();
		this.oInitialSize.height = this.oEditor.jqImgNode.height();

		this.oEditor.hide();
		this.oEditor.redrawMenu([ApplyModule, CancelModule]);
		this.oEditor.jqMenuWrapper.css({
			bottom: -15 - this.oEditor.jqMenuWrapper.height() + 'px'
		});
		this.oEditor.removeCloseButton();
		this.oEditor.switchOff();
		this.oEditor.show();
		...
		this.setSelection();
		tinymce.activeEditor.on('ObjectResized', function(oEvent){
			self.oEditor.applyImageSizes();
			self.oEditor.reposition();
			self.setSelection();
		});
	};

	ResizeModule.prototype.save = function () {
		ResizeModule.superclass.deactivate.call(this);

		var oData = {
			width: this.oEditor.jqImgNode.width(),
			height: this.oEditor.jqImgNode.height()
		};

		this.process('resize', oData);
	...
	ResizeModule.prototype.cancel = function () {
		this.cleanup();
		this.oEditor.jqImgNode.width('').height('').removeAttr('width height');
	};

	ResizeModule.prototype.cleanup = function () {
		this.removeSelection();
		var self = this;
		window.setTimeout(function(){
			if (self.oEditor.getEditState() && (self.oEditor.getEnabledModule() instanceof EditModule === false)) {
				var oEditModule = new EditModule(self.oEditor);
				oEditModule.activate();
			}
		}, 0);
	...
	ResizeModule.prototype.setSelection = function () {
		var self = this;
		this.oEditor.applyImageSizes();
		tinymce.activeEditor.settings.object_resizing = true;
		tinymce.activeEditor.selection.controlSelection.showResizeRect(this.oEditor.jqImgNode[0]);
	};

	ResizeModule.prototype.removeSelection = function () {
		if (tinymce && tinymce.activeEditor) {
			tinymce.activeEditor.selection.controlSelection.hideResizeRect();
			tinymce.activeEditor.settings.object_resizing = false;
		}
		this.oEditor.applyImageSizes();
	};


	function RotateModule (oEditor) {

		RotateModule.superclass.constructor.call(this);

		this.title = getLabel("js-ieditor-module-rotate-title");
		this.cssClass = settings.module.rotate_module.css_class;

		this.init(oEditor);

	};
	uAdmin.inherit(RotateModule, ModuleBase);

	RotateModule.isImageEditorModule = true;

	RotateModule.prototype.activate = function () {
		this.oEditor.showPreloader();
		RotateModule.superclass.activate.call(this);
		this.process('rotate');
		this.oEditor.jqImgNode.removeAttr('data-mce-style');
	};
	...
	PopupModule.prototype.activate = function () {
		...
		this.oEditor.reinit();
		this.oEditor.initLayout();
		this.oEditor.setEditState(false);
	...
	PopupModule.changeImage = function (oEditor, isRemove) {
		if (oEditor instanceof ImageEditorBase === false) return;
