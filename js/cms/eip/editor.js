	...
	*/
	...
	uEditor.prototype.bindFinishEvent = function() {
		...
		jQuery(document).on('click', function(e) {
			...
			if(jQuery(e.target).parents().andSelf().hasClass('eip_win')) return true;
			if(jQuery(e.target).parents().andSelf().hasClass('eip-ieditor-img-wrapper')) return true;
			if(jQuery(e.target).parents().andSelf().hasClass('eip-ieditor-imgareaselect-wrapper')) return true;
			if(jQuery(e.target).parents().andSelf().filter('#mce-modal-block').length) return true;
			if(jQuery(e.target).parents().andSelf().filter('#popupLayerScreenLocker').length) return true;
			if(jQuery(e.target).parents().andSelf().filter('#jGrowl').length) return true;
			...
			for(var i = 0; i < parents.size(); i++) {
				...
				if(jQuery(pNode).hasClass("mceMenu")) return true;
				if(jQuery(pNode).hasClass('mceEditor')) return true;
				if(jQuery(pNode).parents().andSelf().hasClass('mceColorSplitMenu')) return true;
				if(jQuery(pNode).parents().andSelf().hasClass('mceMenuItem')) return true;
				if(jQuery(pNode).parents().andSelf().hasClass('mce-panel')) return true;
				if(jQuery(pNode).parents().andSelf().hasClass('eip-ieditor-imgareaselect-wrapper')) return true;
			}
