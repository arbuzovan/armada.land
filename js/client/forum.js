...
function forum_check_reg_form() {
	...
	if(!forum_login || !forum_password || !forum_password_check || !forum_email) {
		alert(getLabel('js-client-forum-not_correct_form'));
		return false;
	}

	if(forum_login.value == "") {
		alert(getLabel('js-client-forum-must_fill_field_login'));
		return false;
	}

	if(forum_password.value == "") {
		alert(getLabel('js-client-forum-must_fill_field_password'));
		return false;
	}

	if(forum_password.value != forum_password_check.value) {
		alert(getLabel('js-client-forum-password_not_correct'));
		return false;
	}

	if(forum_email.value == "") {
		alert(getLabel('js-client-forum-must_fill_field_email'));
		return false;
	}
...
function forum_check_form(obj) {
	...
	if(obj.captcha) {
		if(md5(obj.captcha.value) != getCookie("captcha1")) {
			alert(getLabel('js-client-forum-code_not_correct'));
			return false;
		}
	}
	...
	if(vlogin.length == 0) {
		alert(getLabel('js-client-forum-input_name'));
		return false;
	}
	...
	if(vtitle.length == 0) {
		alert(getLabel('js-client-forum-input_header'));
		return false;
	}
	...
	if(bd.length == 0) { 
		alert(getLabel('js-client-forum-input_message_text'));
		return false;
	}
	return true;
}
