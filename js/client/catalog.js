function catalog_addToCompare(type_id, object_id, lobj, cat_id) {
	...
	res.innerHTML = "<span style='font-size: 10px;'>" + getLabel('js-client-catalog-add_to_comparison') + "</span><br />";
	...
	if(cl = document.getElementById('catalogEarseCompareLink')) {
		...
		res.innerHTML += "(<a href=\"" + url + "\" onclick=\"" + clk + "\" style='font-size: 10px;'><u>" + getLabel('js-client-catalog-compare') + "</u></a>)";
	}
...
//	var url = "/eshop/addToCart/" + item_id + "/";
	...
	if(obj = document.getElementById('addToCart_txt_' + item_id)) {
		obj.innerHTML = getLabel('js-client-catalog-added_in_cart') +
		'<br /><a href="/eshop/my_cart/">(' + getLabel('js-client-catalog-in_cart') + ')</a>';
	}
}
